...
function Scene() {
  ...
  // phones view the same scene scaled down as one group, so the full curve fits
  // the portrait viewport while keeping desktop pixel-for-pixel unchanged
  const sceneScale = isMobile ? 0.26 : 1;
  ...
  useFrame((_, dt) => {
    ...
    // ride the curve: the camera motion is scaled with the scene on mobile, so
    // it moves just like desktop, only smaller — never frozen, never panning off
    const camX = (prog - 0.5) * 3 * sceneScale + pointer.x * 0.8;
    const camY = pointer.y * 0.5 + (prog - 0.5) * 0.7 * sceneScale;
    const camZ = (isMobile ? 9.4 : 8.6) - Math.sin(prog * Math.PI) * (isMobile ? 0.5 : 1.1);
